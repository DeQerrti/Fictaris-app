...
// bg — цвет полосы и системных кнопок берётся из --panel, а не --bg:
// тот же цвет, что у сайдбара, отличимый от фона самого содержимого
// (--bg) — раньше полоса красилась в --bg и на однотонных экранах
// (рукопись, пустой холст) сливалась с текстом под ней, будто её нет.
export function overlayColors(skin) {
  return skin === "light" ? { bg: "#ece0c8", symbol: "#5c4d34" } : { bg: "#1e1912", symbol: "#a99977" };
}
...
export function titleBarCss() {
  return `
    ...
    /* Полоса, за которую окно таскают мышью. Фон — --panel, тот же, что
       у сайдбара, а не --bg (фон самого содержимого): рамка остаётся
       частью общей темы, но отличима от того, что под ней — раньше
       оба были одним и тем же --bg, и полоса пропадала из виду на
       однотонных экранах (рукопись, пустой холст и т.п.). Фолбэк на
       #1e1912 — для экрана приветствия (electron/ui/welcome.html), у
       него своя разметка без переменной --panel. */
    html::before {
      content: "";
      position: fixed;
      top: 0; left: 0; right: 0;
      height: var(--app-titlebar);
      background: var(--panel, #1e1912);

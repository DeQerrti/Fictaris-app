...
// bg — цвет полосы и системных кнопок берётся из --card, на ступень
// светлее и --bg (фон содержимого), и --panel (фон сайдбара) — раньше
// пробовали --panel, но попросили ещё светлее, чтобы полоса точно не
// сливалась ни с чем под ней.
export function overlayColors(skin) {
  return skin === "light" ? { bg: "#faf3e4", symbol: "#5c4d34" } : { bg: "#2a2318", symbol: "#a99977" };
}
...
export function titleBarCss() {
  return `
    ...
    /* Полоса, за которую окно таскают мышью. Фон — --card, на ступень
       светлее и --bg (фон содержимого), и --panel (фон сайдбара) —
       рамка остаётся частью общей темы, но не сливается ни с чем под
       собой. Фолбэк на #2a2318 — для экрана приветствия
       (electron/ui/welcome.html), у него своя разметка без переменной
       --card. */
    html::before {
      content: "";
      position: fixed;
